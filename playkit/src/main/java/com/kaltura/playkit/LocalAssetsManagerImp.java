...
public class LocalAssetsManagerImp extends LocalAssetsManager {
    ...
    public void registerWidevineDashAsset(String assetId, String licenseUri, byte[] drmInitData) throws RegisterException {
        final WidevineModularAdapter widevine = new WidevineModularAdapter(context, localDataStore);
        widevine.registerAsset(drmInitData, "video/mp4", licenseUri, licenseRequestParamAdapter);
        localDataStore.save(buildAssetKey(assetId), buildMediaFormatValueAsByteArray(PKMediaFormat.dash, PKDrmParams.Scheme.WidevineCENC));
    }
    ...
    public PKMediaSource getLocalMediaSource(@NonNull final String assetId, @NonNull final String localAssetPath) {
        return new LocalMediaSourceImp(localDataStore, localAssetPath, assetId);
    }
...
//     * @param exoMediaSource - the actual url of the video that should be played.
    ...
    public static class LocalMediaSourceImp extends LocalMediaSource {
         ...
         */
        LocalMediaSourceImp(LocalDataStore localDataStore, String localPath, String assetId) {
            setId(assetId);
            setUrl(localPath);
            this.localDataStore = localDataStore;
